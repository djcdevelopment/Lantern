...
const RaidsListPage = ({ setRoute }) => {
  const raids = window.RAID_HISTORY;
  return (
    <div className="page">
      ...
      <div className="raid-list">
        {raids.map(r => (
          <div key={r.id} className="raid-row" onClick={() => setRoute("raids/2026-05-12")}>
            <div className="date">
              {r.day.toUpperCase()}
            ...
            <div className="stats">
              <span>{r.deaths === 0 ? "no deaths" : <><b>{r.deaths}</b> death{r.deaths !== 1 && "s"}</>}</span>
...
window.RaidsListPage = RaidsListPage;
